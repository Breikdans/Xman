...
void SoundFX::loadImpl()
{
	...
	if (_path == "")
	{
		...
		throw (Ogre::Exception(Ogre::Exception::ERR_FILE_NOT_FOUND, "Imposible encontrar el recurso", _path.c_str(), "SoundFX::loadImpl()", __FILE__, __LINE__));
	}
	...
	if ((_pSound = Mix_LoadWAV(_path.c_str())) == NULL)
	{
		...
		throw (Ogre::Exception(Ogre::Exception::ERR_INTERNAL_ERROR, "Imposible cargar el efecto", _path.c_str(), "SoundFX::loadImpl()", __FILE__, __LINE__));
	}
