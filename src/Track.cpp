...
void Track::loadImpl()
{
	...
	if (_path == "")
	{
		...
		throw (Ogre::Exception(Ogre::Exception::ERR_FILE_NOT_FOUND, "Archivo no encontrado", "Track::loadImpl()"));
	}
...
std::cout << "PLAYING: " << _path.c_str() << std::endl;
	...
	else	// Si no, se reproduce desde el principio.
	{
		if (Mix_PlayMusic(_pTrack, loop) == -1)
		{
			pLogManager->logMessage("Track::play() Error al reproducir el recurso de sonido.");
			throw (Ogre::Exception(Ogre::Exception::ERR_FILE_NOT_FOUND,	"Imposible reproducir el recurso de sonido", "Track::play()"));
		}
	}
}
...
void Track::fadeIn(int ms, int loop)
{
	...
	if (Mix_FadeInMusic(_pTrack, ms, loop) == -1)
	{
		...
		throw (Ogre::Exception(Ogre::Exception::ERR_INTERNAL_ERROR,	"Imposible aplicar suavizado de sonido", "Track::fadeIn()"));
	}
}
